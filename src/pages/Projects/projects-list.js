...
const columns = [
  ...
  },
  {
    ...
    render: team => (
      <>
      {team ?  <Tooltip title={`${team.name}`} placement="top"><Avatar
          style={{
...
const ProjectsList = () => {
  ...
  const fetchData = () => {
    setLoading(true);
    let url = GET_PROJECTS + "?page=" + (tableParams.pagination.current - 1) + "&size=" + tableParams.pagination.pageSize + ((tableParams.filters && tableParams.filters.status ? ("&status=" + tableParams.filters.status) : "") + (myProject? ("&accountId="+accountId) : ""))
    get(url)
      .then((results) => {
        console.log(results);
    ...
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid style={{overflowX:"scroll"}}>
          ...
          <Row>
            <Checkbox checked={myProject} onChange={myProjectChange}>My Projects</Checkbox>
          </Row>
          <Row>
            <Table
              columns={columns}
